...
class UNetConvLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(S):
            ...
            x2 = self.down1(x1)
            x3 = self.down2(x2)
            encoded_features.append((x1, x2, x3))
        
        encoded_stack = torch.stack([feat[2] for feat in encoded_features], dim=1)
        ...
        x = self.up1(bottleneck_out, encoded_features[-1][2])
        x = self.up2(x, encoded_features[-1][0])
        x = self.outc(x)
        return x 